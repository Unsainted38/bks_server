...
void TcpTransport::setupTransport() {
    ...
    connect(socket, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(onErrorOccured(QAbstractSocket::SocketError)));
...
void TcpTransport::onErrorOccured(QAbstractSocket::SocketError err) {
    Q_UNUSED(err);
